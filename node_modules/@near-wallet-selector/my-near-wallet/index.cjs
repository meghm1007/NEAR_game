...
objectGetOwnPropertyDescriptor.f = DESCRIPTORS$e ? $getOwnPropertyDescriptor$1 : function getOwnPropertyDescriptor(O, P) {
  ...
  if (hasOwn$b(O, P)) return createPropertyDescriptor$4(!call$d(propertyIsEnumerableModule$1.f, O, P), O[P]);
};
...
var v8PrototypeDefineBug = DESCRIPTORS$d && fails$a(function () {
  // eslint-disable-next-line es/no-object-defineproperty -- required for testing
...
var TypeError$4 = globalThis$c.TypeError;
...
var CONFIGURABLE_LENGTH = DESCRIPTORS$9 && !fails$9(function () {
...
var NEW_ITERATOR_PROTOTYPE = !isObject$6(IteratorPrototype$2) || fails$3(function () {
  var test = {};
...
var environment = (function () {
  ...
  if (classof$4(globalThis$a.process) === 'process') return 'NODE';
...
var exec$3 = uncurryThis$9(constructorRegExp.exec);
...
var bind$7 = uncurryThis$7(uncurryThis$7.bind);
...
var safeGetBuiltIn$2 = function (name) {
  if (!DESCRIPTORS$4) return globalThis$8[name];
...
if (!microtask$1) {
  ...
  // - setTimeout
    ...
    macrotask = bind$5(macrotask, globalThis$7);
    notify$1 = function () {
      macrotask(flush);
    };
  }
...
var TypeError$3 = globalThis$4.TypeError;
...
var onUnhandled = function (state) {
  call$8(task, globalThis$4, function () {
...
class MyNearWalletConnector {
  constructor(walletUrl, network) {
    ...
    if (walletAuthKey) {
      ...
      const privateKey = localStorage.getItem(`near-api-js:keystore:${accountId}:${network.networkId}`);
      if (privateKey) {
        const {
          contractId,
          methodNames
        } = JSON.parse(localStorage.getItem("near-wallet-selector:contract") || "{}");
        this.functionCallKey = {
          privateKey: privateKey,
          contractId,
          methods: methodNames || []
        };
        localStorage.setItem("functionCallKey", JSON.stringify(this.functionCallKey));
        localStorage.removeItem(`near-api-js:keystore:${accountId}:${network.networkId}`);
      }
    }
  ...
  getPublicKey() {
    if (this.functionCallKey) {
      return utils.KeyPair.fromString(this.functionCallKey.privateKey).getPublicKey();
    }
    return undefined;
  }
  isSignedIn() {
  ...
  requestSignIn({
    ...
    return __awaiter(this, void 0, void 0, function* () {
      const url = yield this.requestSignInUrl({
        contractId,
        methodNames
      });
      return yield this.handlePopupTransaction(url, data => __awaiter(this, void 0, void 0, function* () {
  ...
  signMessage({
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      return yield this.handlePopupTransaction(href.toString(), value => {
        var _a, _b, _c;
        return {
          accountId: ((_a = value === null || value === void 0 ? void 0 : value.signedRequest) === null || _a === void 0 ? void 0 : _a.accountId) || "",
          publicKey: ((_b = value === null || value === void 0 ? void 0 : value.signedRequest) === null || _b === void 0 ? void 0 : _b.publicKey) || "",
          signature: ((_c = value === null || value === void 0 ? void 0 : value.signedRequest) === null || _c === void 0 ? void 0 : _c.signature) || ""
        };
      });
    });
  }
  signAndSendTransactions(transactionsWS) {
    return __awaiter(this, void 0, void 0, function* () {
      const txs = yield Promise.all(transactionsWS.map(t => this.completeTransaction(t)));
  ...
  signAndSendTransaction({
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const tx = yield this.completeTransaction({
        receiverId,
        actions
      });
      const results = yield this.signAndSendTransactionsMNW([tx]);
      return results[0];
    });
  }
  completeTransaction({
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const block = yield this.provider.block({
        finality: "final"
      });
      const blockHash = utils.serialize.base_decode(block.header.hash);
      // create Transaction for the wallet
      return nearApiJs.transactions.createTransaction(this.signedAccountId, utils.KeyPair.fromRandom("ed25519").getPublicKey(), receiverId, 0, actions.map(a => walletUtils.createAction(a)), blockHash);
    });
  }
  signAndSendTransactionsMNW(txs) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
      const url = this.requestSignTransactionsUrl(txs);
      const txsHashes = (_a = yield this.handlePopupTransaction(url, data => data.transactionHashes)) === null || _a === void 0 ? void 0 : _a.split(",");
      if (!txsHashes) {
  ...
  storedKeyCanSign(receiverId, actions) {
    if (this.functionCallKey && this.functionCallKey.contractId === receiverId) {
      return actions[0].type === "FunctionCall" && actions[0].params.deposit === "0" && (this.functionCallKey.methods.length === 0 || this.functionCallKey.methods.includes(actions[0].params.methodName));
    } else {
      return false;
    }
  }
  signUsingKeyPair({
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      const keyPair = utils.KeyPair.fromString(this.functionCallKey.privateKey);
      const signer = yield nearApiJs.InMemorySigner.fromKeyPair(this.network.networkId, this.signedAccountId, keyPair);
      const connection = new nearApiJs.Connection(this.network.networkId, this.provider, signer, "");
      const account = new nearApiJs.Account(connection, this.signedAccountId);
      return account.signAndSendTransaction({
        receiverId,
        actions: actions.map(a => walletUtils.createAction(a))
      });
    });
  }
  requestSignTransactionsUrl(txs) {
    const newUrl = new URL("sign", this.walletUrl);
    newUrl.searchParams.set("transactions", txs.map(transaction$1 => borsh__namespace.serialize(transaction.SCHEMA.Transaction, transaction$1)).map(serialized => Buffer.from(serialized).toString("base64")).join(","));
    newUrl.searchParams.set("callbackUrl", new URL(window.location.href).toString());
    return newUrl.toString();
  }
  handlePopupTransaction(url, callback) {
    return __awaiter(this, void 0, void 0, function* () {
      const screenWidth = window.innerWidth || screen.width;
      const screenHeight = window.innerHeight || screen.height;
      const left = (screenWidth - DEFAULT_POPUP_WIDTH) / 2;
      const top = (screenHeight - DEFAULT_POPUP_HEIGHT) / 2;
      const childWindow = window.open(url, "MyNearWallet", `width=${DEFAULT_POPUP_WIDTH},height=${DEFAULT_POPUP_HEIGHT},top=${top},left=${left}`);
      if (!childWindow) {
  ...
  setupMessageHandler(resolve, reject, childWindow, callback) {
...
const MyNearWallet = ({
  ...
  const state = yield setupWalletState(params, options.network);
  ...
  return {
    ...
    signMessage({
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        return yield state.wallet.signMessage({
          message,
          nonce,
          recipient,
          callbackUrl,
    ...
    signAndSendTransactions({
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        return state.wallet.signAndSendTransactions(transactions);
      });
    },
